...
- **막힌 결정**: Step 2.5 의 자격 판정 한 줄 → 전제 목록 P1..Pn **그대로**(C2) → builder 의 `premise_list_challenge`
  원문.
- **추천 답안**: 두 줄 나란히 — 「builder: <recommendation>」 / 「orchestrator: <판정> — <이유>」(판정은 네 선택지 중 하나).
...
- **질문**: `AskUserQuestion` 선택지 **고정 순서** 유지 / 보완 / 전환 / 보류 — 추천을 첫 자리로 옮기지 않는다. 추천받은 선택지 라벨 뒤에 출처를 붙인다: builder 만 추천하면 `(builder 추천)`, orchestrator 만 추천하면 `(orchestrator 추천)`, 둘이 같으면 `(builder·orchestrator 추천)`. builder 추천이 switched 이고 Step 2.5 가 `재검토 사유 없음` 이면 전환 라벨은 `(builder 추천 · 전제 충돌 없음)` 이다. builder 의 kept / refined / switched 는 유지 / 보완 / 전환이다.
