...
# 줄에 함께 있어야 하고, 추천 답안이나 Step 2.5 로 흩어지면 RED. 리터럴은 조건과 라벨을 **동사까지 묶어**
# 잰다 — 라벨만 재면 두 출처의 라벨을 맞바꾸거나 조건을 지우거나 뒤집어도 통과한다. 질문 규칙은
# steelman.md 에서 **한 물리 줄**이어야 한다 — 줄바꿈하면 뒷부분이 코퍼스에서 빠져 RED.
...
# 라벨의 봉쇄가 기대는 전제다. 코퍼스는 R3 블록이 아니라 그 항목 하나다: 블록 전체로 재면 문장을 뒤집거나
# 충돌 ≥1 항목으로 옮겨도 통과한다.
...
for lit in \
  ...
  '그 옆에 `[전제 충돌 없음]` 라벨을 붙인다'; do
  grep -qF "$lit" <<<"$c0_block" && ok "R3 추천 표시 충돌 0 항목: $lit" || no "R3 추천 표시 충돌 0 항목에 없음: $lit"
done
