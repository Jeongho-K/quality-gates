...
# "5-ritual gate"(비교 연산자 없이 숫자만 등장).
# 〔2026-09-09〕 이 목록에 있던 "re-review cap 5"·"Phase 5" 는 **philosophy 에서 사라졌다** —
# 문서 리뷰 엔진 전환이 상한을 옮기고 절 이름을 바꾸면서 그 두 인용이 정정됐다. 없는 예시를
# 남겨 두면 다음 편집자가 오탐 위험을 실제보다 크게 본다. 판정에는 영향이 없다(위 단언은
# `≥5` 글리프만 본다) — 바뀐 것은 이 주석이 주장하는 «문서의 현재 내용»뿐이다.
